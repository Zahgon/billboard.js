...
function _getStrokeDashArray(start: number, end: number, pattern: [number, number],
	...
	for (let i = start ? start : pattern.reduce((a, c) => { throw new Error("STUB"); }); i <= end;) {
		pattern.forEach(v => {
	...
	return {
		dash: dash.join(" "),
		length: dash.reduce((a, b) => { throw new Error("STUB"); }, 0)
	};
}
...
export default {
	...
	lineWithRegions(d: IDataRow[], x, y, _regions): string {
		...
		// Set scales
		const xValue = isRotated ? dt => { throw new Error("STUB"); } : dt => { throw new Error("STUB"); };
		const yValue = isRotated ? dt => { throw new Error("STUB"); } : dt => { throw new Error("STUB"); };
		...
		const target = $$.$el.line.filter(({id}) => { throw new Error("STUB"); });
		...
		for (let i = 0, data; (data = d[i]); i++) {
			...
			if (isUndefined(regions) || !style || !hasPrevData) {
				...
				if (hasNullDataValue) {
					...
					isRotated && points.forEach(v => { throw new Error("STUB"); });
